...
class _MesCommandesState extends State<MesCommandes> {
  ...
  var previous;
   ...
   Future getcommande() async{
    ...
    var url=Uri.parse('https://gaalguishop.herokuapp.com/api/produit/commandeencours/',);
...
Future getothercommande() async{
 setState(() {
  load=true;
    ...
    var url=Uri.parse('https://gaalguishop.herokuapp.com/api/produit/commandeencours/?page=${page}',);
    var result=await  httpIns.get(url);
    if(result.statusCode==200){
  var jsonResponse = convert.jsonDecode(result.body) as Map<String, dynamic>; 
  //print(result.body); 
          ...
          items.isNotEmpty?
           Expanded(
                ...
                return Container(
                 margin: const EdgeInsets.only(bottom: 5),
                  child:  Card(
                    ...
                    Text("Etat de la commande: ${item['statut_commande']}")
                       ], 
                    ),
                  )
              ); }
            ...
            child: Center(child: Column(
              children: [
              const Text("Vous n avez aucune commande,"),
